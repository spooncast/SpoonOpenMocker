...
- 상태: 구현 완료
- 갱신일: 2026-06-24
- 관련: [flows/wire-injector-to-app-feature](../flows/wire-injector-to-app-feature.md)

## 목적
"IDE 플러그인으로 WebSocket 메시지 테스트가 가능한가"를 데모 앱에서 **눈으로 확인**할 수 있게 한다.
이 기능은 injector 주입을 **실제 WS 메시지 스트림**으로 이어 데모 화면(Realtime 탭)에 실시간 표시함으로써,
플러그인 Inject → 앱 수신 경로를 관찰 가능하게 만든다.
...
- **데모 구현(`DemoChatSocketClient`)**: OpenMocker injector(`DemoEventInjector`)가 주입한 payload 를
...
```
[Plugin Event Injection 탭] → POST /inject/demo → EventInjectorRegistry
  → DemoEventInjector.inject(payload) → DemoChatSocketClient.emit(payload)
    → incoming: SharedFlow<String> → WsViewModel → WsPane(메시지 리스트)
```

## 사용자 시나리오
- 개발자가 IDE 플러그인 Event Injection 탭에서 injector 를 고르고 payload 를 Inject 하면, 데모 앱 Realtime 탭에
  해당 메시지가 즉시 나타난다.
...
2. `DemoChatSocketClient`(`@Singleton`): `emit(payload)` 가 `incoming` 으로 메시지를 흘려보낸다. injector/ViewModel 이 같은 싱글톤 공유.
3. `DemoEventInjector`(`BufferedEventInjector`, id=`"demo"`): `deliver(payload)` → `client.emit(payload)`. `name="Realtime (WebSocket) Demo"`. 수신 이력(recorded)·sequence 채번은 베이스가 처리.
4. `DemoApplication`(debug): `EntryPointAccessors.fromApplication` 으로 싱글톤 client 를 꺼내 `OpenMocker.control.registerInjector(DemoEventInjector(client))`.
...
- **라이브러리는 실제 WebSocket 프레임을 가로채지 않는다.** HTTP 처럼 요청 경로에 앉는 인터셉터가 아니라,
  injector 는 **out-of-band 주입 통로**다. 해석/모킹 책임은 전적으로 앱(injector 구현)에 있다.
