#!/usr/bin/env python3
"""Create a git worktree for this project (OpenMocker) and make it gradle-ready.

A fresh `git worktree` does not receive git-ignored files, so `local.properties`
(the Android SDK path) is missing in the new worktree and gradle cannot locate
the SDK to configure the build. This script creates the worktree and copies that
config file over so the worktree builds immediately.

Two modes:
  - Existing branch:  create_worktree.py <branch>
  - New branch:       create_worktree.py <type>/<issue>-<slug> --base <baseref>
...
# OpenMocker only needs the SDK path in local.properties (no secrets.properties,
# no JFROG credentials — settings.gradle.kts evaluates without them).
CONFIG_FILES = ["local.properties"]
...
def main():
    ...
    if args.path:
        ...
        worktree_path = source_root.parent / f"{source_root.name}-worktrees" / dir_name
